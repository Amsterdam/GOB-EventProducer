        ...
        stage('Test') {
            tryStep "test", {
                sh "docker-compose -p GOB-KafkaProducer_service -f src/.jenkins/test/docker-compose.yml build --no-cache && " +
                   "docker-compose -p GOB-KafkaProducer_service -f src/.jenkins/test/docker-compose.yml run -u root --rm test"
                ...
                tryStep "deployment", {
                    build job: 'Subtask_Openstack_Playbook',
                        parameters: [
                            ...
                            [$class: 'StringParameterValue', name: 'PLAYBOOK', value: 'deploy-gob-kafkaproducer.yml'],
                        ]
                }
            }
        }
